...
class IssueFetcher:
    ...
    async def fetch_issues_with_code(self, num_issues: int = 10) -> List[ExtractedIssue]:
        ...
        print(f"  Fetching closed issues from {len(TYPE_CHECKER_REPOS)} repositories...")

        for repo, issue_tracker_url in TYPE_CHECKER_REPOS.items():
            print(f"    Fetching from {repo}...")
            print(f"      Issue tracker: {issue_tracker_url}")
                ...
                if code_blocks:
                    ...
                    if code and len(code) > 20:
                        all_issues.append(ExtractedIssue(
                            ...
                            title=issue["title"],
                            original_code=code,  # Direct extraction, no LLM
                            issue_description=issue.get("body", "")[:1000],
                            labels=[l["name"] for l in issue.get("labels", [])],
                        ))

            print(f"      Found {len([i for i in all_issues if i.repo == repo])} issues with code")
        ...
        print(f"  Total issues selected: {len(selected)}")
        return selected
...
class FalseResultDiscoveryPipeline:
    ...
    async def run(self, num_examples: int = 10) -> List[Dict]:
        """Run the pipeline"""
        print(f"Starting pipeline (max {self.max_rounds} rounds per issue)...")

        # Stage 1 & 2: Fetch issues and extract code
        print("\n[Stage 1-2] Fetching closed issues and extracting code...")
        issues = await self.issue_fetcher.fetch_issues_with_code(num_examples)
        
        if not issues:
            print("ERROR: No issues found with code examples")
            return []
        ...
        for i, issue in enumerate(issues, 1):
            print(f"\n{'='*60}")
            print(f"[Issue {i}/{len(issues)}] {issue.title[:50]}...")
            print(f"  Issue tracker: {issue.issue_tracker_url}")
            print(f"  Source: {issue.issue_url}")
            print(f"  Original code: {len(issue.original_code)} chars")
            
            # Check original code first
            print("  [Pre-check] Validating original code...")
            ...
            if original_validation.has_divergence:
                print(f"    ✓ Original code already has divergence!")
                print(f"    Results: {original_validation.checker_classifications}")
                results.append({
                ...
                print(f"    Original: {original_validation.checker_classifications}")

            # Stage 3: Initial tweak
            print("  [Stage 3] Creating initial tweak...")
            try:
                example = await self.code_tweaker.tweak_code(issue)
                print(f"    Tweak: {example.tweak_description[:60]}...")
            except Exception as e:
                print(f"    ERROR: {e}")
                continue
            ...
            while round_num < self.max_rounds and not divergence_found:
                round_num += 1
                print(f"  [Round {round_num}/{self.max_rounds}] Running type checkers...")
                
                validation = self.validator.validate(example)

                # Handle syntax errors
                if not validation.is_valid_syntax:
                    print(f"    ⚠ Syntax error, fixing...")
                ...
                # Check results
                print(f"    Results: {validation.checker_classifications}")

                if validation.has_divergence:
                    print(f"  ✓ DIVERGENCE FOUND in round {round_num}!")
                    print(f"    {validation.analysis}")
                    divergence_found = True
                else:
                    print(f"    {validation.analysis}")
                    if round_num < self.max_rounds:
                        print(f"    Tweaking again...")
            ...
            if divergence_found:
                ...
                print(f"  ✗ No divergence after {round_num} rounds")
        ...
        print(f"Divergent examples found: {len(results)}")
        if results:
            avg_rounds = sum(r['rounds'] for r in results) / len(results)
            print(f"Average rounds to divergence: {avg_rounds:.1f}")
...
def print_results(results: List[Dict]) -> None:
    ...
    for r in results:
        print(f"\n{r['id']}")
        print(f"Issue Tracker: {r.get('source_issue_tracker', 'N/A')}")
        print(f"Source Issue: {r['source_issue']}")
        print(f"Rounds: {r['rounds']}")
        print(f"Tweak: {r['tweak_description']}")
